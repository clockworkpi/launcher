...
class PageListItem(InfoPageListItem):
    ...
    def Draw(self):
        ...
        if self._Active == True:
            self._Parent._Icons["done"].NewCoord(
                self._Parent._Width-30, self._PosY+5)
            self._Parent._Icons["done"].Draw()

        self._Labels["Text"].Draw(self._Active)

        if "Small" in self._Labels:
            self._Labels["Small"]._PosX = self._Width - \
                self._Labels["Small"]._Width - 10
            self._Labels["Small"]._PosY = self._PosY + \
                (self._Height - self._Labels["Small"]._Height)/2
            self._Labels["Small"].Draw()

        pygame.draw.line(self._Parent._CanvasHWND, MySkinManager.GiveColor(
            'Line'), (self._PosX, self._PosY+self._Height-1), (self._PosX+self._Width, self._PosY+self._Height-1), 1)
...
class WallpaperPage(Page):
    ...
    def GenList(self):
        ...
        for i, u in enumerate(content):
            ...
            li.Init(image_name.decode("utf8"))
        ...
        no_wallpaper = PageListItem()
        no_wallpaper._Parent = self
        no_wallpaper._PosX   = start_x
        no_wallpaper._PosY   = start_y + last_height
        no_wallpaper._Width  = Width
        no_wallpaper._Fonts["normal"] = self._ListFont
        no_wallpaper._Active = False
        no_wallpaper._Value = 'None'
        no_wallpaper._Path = 'None'
        no_wallpaper.Init('None')
        self._MyList.append(no_wallpaper)
...
class APIOBJ(object):
    ...
    def Init(self,main_screen):
        self._Page = WallpaperPage()
        self._Page._Screen = main_screen
        self._Page._Name ="Wallpaper"
        self._Page.Init()
